...
| 界面 | 启动命令 | 适用场景 |
| --- | --- | --- |
| 现代分析工作台 | `python main.py modern` | 浏览器工作流、丰富图表、实时抓包、网络拓扑、端点安全和 LLM 建议 |
...
`python modern_main.py` 仍作为兼容快捷入口保留。运行 `python main.py --help` 可以查看所有整合后的启动模式。
...
需要进行本机实时演示时，在 Traffic Monitor 中保留 Default interface，并运行 `python main.py demo` 启动 [HTTP 告警演示实验室](demo_http_lab/README.md)。原有 `python demo_http.py` 快捷入口仍然可用。控制页面只监听 `127.0.0.1`，提交的无害明文帧会注入终端打印的默认网卡，并经过正常实时抓包和 Detection Engine；内容不会被执行，也不会发送到公网目标。
...
| `demo_http_lab/` | 私网 HTTP 接收器和最小告警演示页面 |
