...
启动器会在 `127.0.0.1:8787` 启动本地 API，在 `127.0.0.1:4173` 启动前端。如果页面提示 `Offline preview`，请重新启动 `modern_main.py`；预览数据不是项目中的持久化数据。
...
Assets 可以为唯一 IP 设置显示名称、受控角色、0 到 100 的重要性和备注。资产重要性会影响人工处置优先级和主机风险评分，但不会自动创建白名单。由于本地 API v5 暂未提供 Assets CRUD 路由，目前持久化资产编辑需要使用经典桌面端。
...
调查记录包含标题、主机、状态、优先级、摘要和备注。经典调查证据以快照形式保存，因此删除原告警或重置运行统计后仍然存在。现代 Investigations 页面已经存在，但本地 API v5 尚未提供对应持久化调用；需要长期保存调查和证据时请使用经典桌面端。
...
现代主题、字号、壁纸和悬浮宠物设置保存在浏览器 local storage 中。经典桌面端会把选择的壁纸和宠物文件复制到受管理的个性化资源目录，并在下次启动时恢复。
