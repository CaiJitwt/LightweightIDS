...
需要从虚拟机向宿主机进行实时演示时，请参考 [HTTP 告警演示实验室](../demo_http_lab/README.md)。在连接虚拟机的网卡上使用 `tcp.dstport == 8080` 启动抓包，再从虚拟机打开带令牌的实验页面。样本会经过实时抓包和 Detection Engine，接收器不会执行、持久化或转发内容。
