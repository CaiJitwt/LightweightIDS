...
For a local live demonstration, follow [HTTP Alert Demo Lab](../demo_http_lab/README.md). Leave Default interface selected with `tcp.dstport == 8080`, run `python demo_http.py`, and open `http://127.0.0.1:8080/` on the same computer. The terminal prints the adapter receiving submitted inert frames. Samples pass through live capture and Detection Engine without execution or delivery to a public target.
