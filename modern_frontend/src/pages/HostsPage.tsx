    ...
    <div className="page-stack">
      ...
      <div className="host-workspace">
        ...
        <section className="host-detail" aria-label="Host profile details">
          <header className="host-detail-header"><div><span className="eyebrow">{t("hosts.profile")}</span><h2>{selected.name}</h2><p>{selected.ip} - {selected.role}</p></div><span className={`risk-score large risk-${selected.risk >= 80 ? "high" : selected.risk >= 50 ? "medium" : "low"}`}>{selected.risk}</span></header>
          ...
          <div className="host-analysis">
            ...
            <div className="connection-map"><h3>{t("hosts.recentConnections")}</h3><div className="topology"><span className="topology-host">{selected.ip}<br /><small>{selected.role}</small></span>{profile.connections.slice(0, 3).map((connection) => <div className="topology-peer" key={`${connection.peer}-${connection.protocol}-${connection.port}`}><i /><span>{connection.peer}<br /><small>{connection.protocol}{connection.port ? `:${connection.port}` : ""}</small></span></div>)}</div><div className="connection-events">{profile.timeline.slice(0, 4).map((event, index) => <p key={`${event.timestamp}-${index}`}><strong>{event.timestamp}</strong>{event.type === "Alert" ? "Alert" : event.direction}: {event.summary}</p>)}{!profile.timeline.length && <p>{t("hosts.noActivity")}</p>}</div></div>
          </div>
