    ...
    <div className="page-stack">
      ...
      <div className="host-workspace">
        ...
        <section className="host-detail" aria-label="Host profile details">
          <header className="host-detail-header"><div><span className="eyebrow">{t("hosts.profile")}{profileFallback && " · preview"}</span><h2>{selected.name}</h2><p>{selected.ip} - {selected.role}</p></div><span className={`risk-score large risk-${selected.risk >= 80 ? "high" : selected.risk >= 50 ? "medium" : "low"}`}>{selected.risk}</span></header>
