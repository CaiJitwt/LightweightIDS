...
from detection.tls import tls_metadata_findings
    ...
    def process(self, packet: PacketRecord) -> list[AlertRecord]:
        ...
        return [
            self.create_alert(
                ...
                description=(
                    "TLS handshake metadata contains weak version, weak cipher, certificate, ALPN or SNI indicators."
                ),
                evidence=(
                    f"src_ip={packet.src_ip}; dst_ip={packet.dst_ip}; dst_port={packet.dst_port}; "
                    f"matches={unique_findings}"
                ),
            )
        ]
